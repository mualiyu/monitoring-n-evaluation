{{--
    The state feedback queue (App\Livewire\Oversight\Feedback\FeedbackQueue).

    EVERYTHING A MEMBER OF THE PUBLIC TYPED IS RENDERED WITH {{ }} AND NOTHING
    ELSE — subject, body, submitter name, spam reason. There is no {!! !!} in
    this file and there must never be one.

    Wider than any MDA's queue in one way that matters: comments submitted
    without a project attached appear ONLY here. Someone who wrote about "the
    road by the market" without picking a project from a list still gets read,
    by the one office that can work out whose road it is — which is what the
    "unattached" filter is for.
--}}
@php
    /*
        <x-ui.badge>'s map is keyed by other modules' vocabularies ('pending'
        there means "report not yet filed"), so the key below is chosen for its
...
<div>
    <x-ui.page-header
        ...
        :description="__('Every comment the public has sent about a state project, plus the ones nobody owns. Nothing reaches the portal until a moderator publishes it, and refusing to publish goes on the record with a reason.')"
    />
    ...
    <div class="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        @foreach (\App\Enums\FeedbackStatus::cases() as $case)
            <x-ui.stat
                ...
                :hint="$case === \App\Enums\FeedbackStatus::Pending ? __('state-wide') : null"
            />
        @endforeach
    </div>
        ...
        <div class="p-4">
            ...
            <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
                <div class="flex flex-wrap items-center gap-x-6 gap-y-3">
                    <x-ui.form.checkbox
                        name="unattached"
                        :label="__('No project attached')"
                        :description="__('Visible on this surface only — no entity can see these.')"
                        wire:model.live="unattached"
                    />

                    <x-ui.form.checkbox
                        name="flagged"
                        :label="__('Flagged by the spam heuristic')"
                        :description="__('The filter marks, it never rejects.')"
    ...
    <x-ui.card flush>
        ...
        <div wire:loading.delay.long.remove wire:target="search,status,tenantId,flagged,unattached">
            @if ($this->feedback->isEmpty())
                ...
                @else
                    <x-ui.empty-state
                        ...
                        :description="__('Comments arrive from the public portal, and from staff recording what was said at a town hall or on the phone. Publishing projects is what gives people something to comment on.')"
                    >
                        <x-slot:actions>
                            <x-ui.button variant="secondary" icon="globe" :href="route('oversight.publishing.index')">
                                {{ __('Go to publishing') }}
                            </x-ui.button>
                        </x-slot:actions>
                    </x-ui.empty-state>
            ...
            @else
                <x-ui.table
                    :caption="__('Public feedback across every entity, unmoderated first')"
                    class="p-4 sm:p-0"
                    :headings="[__('Comment'), __('Project'), __('Entity'), __('Received'), __('State'), '']"
                >
                    @foreach ($this->feedback as $item)
                        <x-ui.table.row wire:key="state-feedback-{{ $item->ulid }}">
                            ...
                            <x-ui.table.cell :label="__('Project')">
                                @if ($item->project)
                                    <a
                                        href="{{ route('oversight.projects.show', ['ulid' => $item->project->ulid]) }}"
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1.5 text-ink-subtle">
                                        <x-ui.icon name="question-mark-circle" class="size-4 shrink-0" />
                                        {{ __('Not attached') }}
                                    </span>
                                @endif
                            </x-ui.table.cell>
                            ...
                            <x-ui.table.cell :label="__('Entity')">
                                <span class="text-ink-muted">{{ $item->project?->tenant?->name ?? __('State secretariat') }}</span>
                            </x-ui.table.cell>
    ...
    >
        <x-ui.form.group
            name="reason"
            :label="__('Reason')"
            :hint="__('Required. Read by anyone auditing how the state handles public feedback.')"
            required
        >
            <x-ui.form.textarea
                name="reason"
